...
export const Login = () => {
  ...
  return (
    <div>
      <h2 className="text-neutral-500 font-medium text-[14px] text-center">Log in with:</h2>
      ...
      <form onSubmit={handleSubmit} method="POST" className="flex flex-col">
        <div className="flex flex-col gap-6">
          <TextInput name="email" type="email" required placeholder="Email address or username" />
          <TextInput name="password" type="password" required placeholder="Password" />
        </div>
        <Link to="/auth/sign-up" className="text-primary-200 font-medium text-[14px] block w-full text-right mt-4">Forgot your password?</Link>
        <button disabled={status === "loading"} type="submit" className="text-neutral-white bg-primary-400 rounded-full py-4 my-4 font-medium text-[14px] w-full disabled:opacity-70">Log in</button>
      </form>
      <Link to="/auth/sign-up"
        className="text-primary-400 font-medium text-[14px] block w-full text-center my-4">
        <span className="text-neutral-500">Don't have an account? </span> Sign up
      </Link>
